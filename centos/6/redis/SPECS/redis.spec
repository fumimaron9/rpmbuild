# from: https://github.com/remicollet/remirepo/blob/master/redis/redis.spec
...
install -pDm755 src/%{name}-trib.rb %{buildroot}%{_bindir}/%{name}-trib.rb
...
%dir %attr(0755, redis, redis) %{_localstatedir}/db/%{name}
%{_bindir}/%{name}-*
%if 0%{?with_systemd}
